...
export function settleBets(bets: PlacedBet[], winner: number, luckyNumbers: LuckyNumber[] = [], openingBoost = 0) {
  let grossReturn = 0
  let winningStake = 0
  ...
  const openingBonus = openingReward(grossReturn, totalStake(bets), openingBoost)
  grossReturn += openingBonus
  return { openingBonus, grossReturn, winningStake, winners, net: grossReturn - totalStake(bets), multiplierReturn, hitMultiplier }
}
