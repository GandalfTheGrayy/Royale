...
export default function SimulationResearchResults({ research }: { research: SimulationResearch }) {
  ...
  return <section className="sim-research-results">
    ...
    <section className={`sim-smart-search ${research.searchSummary.outcome}`}>
      <b>{research.searchSummary.outcome === "validated" ? "Motor doğrulanmış bir profil buldu" : research.searchSummary.outcome === "not-validated" ? "Motor etkisiz adayı eledi" : "Uygun ayar adayı oluşmadı"}</b>
      <p>{research.searchSummary.candidatesGenerated} farklı geçici profil son listeye kaldı; {research.searchSummary.candidatesValidated} profil bağımsız örnekte sınandı. {research.searchSummary.automaticEscalations > 0 ? research.searchSummary.automaticEscalations + " belirsiz deney otomatik büyütüldü; en derin doğrulama tohum başına " + number(research.searchSummary.maxValidationRunsPerSeed) + " turdu." : "İstatistiksel büyütme gerektiren umut verici belirsiz aday çıkmadı."}</p>
    </section>
    ...
    {research.iterations.length > 0 && <details open className="sim-iteration-list"><summary>Motor kendi denemesini nasıl ilerletti? · {research.iterations.length} adım</summary><p>İlk taramada her ayar için çok düşükten yükseğe dört değer denendi. Sonra en iyi geçici profil üzerinden yeni ayar değerleri ve iki/üç ayarlı birleşimler araştırıldı. Her adım ilk profille aynı arama tohumlarında kıyaslandı; hiçbir adım canlıya kaydedilmedi.</p>{research.iterations.map(step => <article key={step.step}><b>{step.step}. adım · {step.accepted ? "Daha iyi geçici profil bulundu" : "İlerleme durdu"}</b><p>{step.label} RTP: {rtp(step.comparison.before.rtp)} → {rtp(step.comparison.after.rtp)} · {step.changes.length} ayar.</p></article>)}</details>}
